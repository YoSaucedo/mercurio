...
while ($m = $getusers->fetch()) {
	if ($user != $m['sender']) {
		...
		$getavatar = $conn->query("SELECT avatar FROM users WHERE username = '$sender' LIMIT 1"); ?>
			<div class="with">
				<a href="<?php echo url()."/user/message/?with=".$m['sender']."#message".$m['id']; ?>">
					<img class="with" src="<?php echo url()."/user/".$getavatar->fetch()['avatar']; ?>">
					<?php
				...
				<a class="with username" href="<?php echo url()."/user/?u=".$sender; ?>">@<?php echo mroExcerpt($sender, 7); ?></a>
			</div>
		<?php
	}
} if (!isset($_GET['all']) && $allusers->rowCount() > 7) { ?>
	<div class="with more">
		<a href="<?php echo url()."/user/message/?all"; ?>">
			<img class="with" src="<?php echo url()."/assets/img/plane.png"; ?>">
		</a>
		<a class="with username" href="<?php echo url()."/user/message/?all"; ?>">Ver todos</a>
	</div>
<?php } ?>
</div>
